...
/// \brief Reconstruction of heavy-flavour 3-prong decay candidates with KFParticle
///
...
struct HFCandidateCreator3Prong {
  ...
  OutputObj<TH1F> hmass3{TH1F("hmass3", "3-prong candidates;inv. mass (p K #pi) (GeV/#it{c}^{2});entries", 300, 2.1, 2.4)};
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  WorkflowSpec workflow{
    adaptAnalysisTask<HFCandidateCreator3Prong>(cfgc, TaskName{"hf-cand-creator-3prong-kf"}),
    adaptAnalysisTask<HFCandidateCreator3ProngExpressions>(cfgc, TaskName{"hf-cand-creator-3prong-expressions-kf"})};
  const bool doMC = cfgc.options().get<bool>("doMC");
  if (doMC) {
    workflow.push_back(adaptAnalysisTask<HFCandidateCreator3ProngMC>(cfgc, TaskName{"hf-cand-creator-3prong-mc-kf"}));
  }
  return workflow;
}
